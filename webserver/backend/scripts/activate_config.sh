...
if [ "$MODE" == "gateway" ]; then
  ...
  /usr/local/etc/zero2w_80211ah/scripts/mesh_80211s/config_mesh.sh --wifi-ssid "${REG_SSID}" --wifi-password "${REG_PW}" --halow-ssid "${REG_SSID}" --halow-password "${REG_PW}" --optim "${OPTIM}"
  ...
  /usr/local/bin/disp_custom_msg.sh --line2 "Applying settings..." || true
  /usr/local/bin/disp_loading_bar.sh 3 > /dev/null 2>&1 &
  /usr/local/etc/zero2w_80211ah/scripts/mesh_80211s/config_mesh.sh --wifi-ssid "${REG_SSID}_EXT" --wifi-password "${REG_PW}" --halow-ssid "${REG_SSID}" --halow-password "${REG_PW}" --optim "${OPTIM}"
  /usr/local/bin/disp_connecting.sh || true
  nmcli connection down wifi-setup-open || true
  nmcli connection delete wifi-setup-open || true
  /usr/local/bin/disable_mesh.sh
